...
<span class="w-2/3 flex flex-col md:flex-row p-2 md:p-5 mb-10 bg-rp-cream rounded-xl bg-[url(/sun-vector.png)] bg-no-repeat bg-opacity-100">
    <div class="md:w-1/4 flex flex-col items-center justify-center md:mr-4 text-2xl">
        
        <div class="w-full flex justify-center items-center mt-2">
            <img src="/speakers/{speaker.id}.jpg" alt="{speaker.name}" class="w-48 h-48 object-cover rounded-full">
        </div>
    </div>
    <div class="md:w-3/4 flex flex-col font-sans mt-2">
        <div class="flex flex-col items-center text-center text-2xl bg-gradient-to-r from-rp-hot-pink to-rp-blue text-white rounded-xl p-2 mb-2">
            {speaker.name}
        </div>
        <div class="">
            <p class=" decoration-rp-hot-pink pt-2 inline-block pl-2 pr-2 font-bold">
                {speaker.role}
            </p>
        </div>
