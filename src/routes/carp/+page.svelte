	...
	const fetchFilteredAttendees = async (page_to_fetch: number) => {
        let response;
         try {
			...
			response = await fetch(`${$API_URL}/carp/filter?majors=${uri}&years=${[...grad_year_filters].join(',')}&jobs=${[...job_interest_filters].join(',')}&page=${page_to_fetch}`);
	...
	<div class="h-full text-white flex justify-between flex-col my-20">
		...
		<div class="flex flex-row justify-between">
			...
			<div class="justify-self-end my-auto text-gray-100 accent-rp-pink">
				...
				>
					<button on:click={() => (show_filters_modal = false)} class="absolute right-4 text-2xl text-red-500">
				...
				<tbody >
					{#each filtered_attendees as attendee}
						...
						<tr class="bg-opacity-0 hover:bg-opacity-10 bg-black duration-300">
							...
							<td contenteditable="false" class =" w-56 p-2">{major_dict.get(attendee.studentInfo.major)}</td>
		...
		<div class="pt-3 flex flex-row px-32 justify-between mx-2">
			{#if page != first_page}
				<button aria-label="previous page" title="previous page" on:click={async () => {prev_page()}}>
